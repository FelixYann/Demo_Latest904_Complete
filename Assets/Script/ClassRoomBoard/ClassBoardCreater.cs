...
public class ClassBoardCreater : MonoBehaviour, IEdgeInterface
{
    ...
    private void Awake()
    {
        ...
        boardHolder.transform.position = new Vector3(500, 0, 0);
        roomCreater.roomHolder.transform.position = new Vector3(500, 0, 0);
    }
    ...
    void FurnishTheRooms()
    {
        ...
        foreach (var item in rooms)
        {
            ...
            {
                roomCreater.FurnishRoom(new Vector2(item.xPos, item.yPos), item.roomWidth, item.roomHeight, roomNumber);
            }
            roomNumber++;
        }
    }
    ...
    public List<Vector2>[] GetRoomEdgeAndInterier(int roomNumber)
    {
        ...
        List<Vector2>[] result = new List<Vector2>[9];
        result[0] = up;
        result[1] = down;
        result[2] = left;
        result[3] = right;
